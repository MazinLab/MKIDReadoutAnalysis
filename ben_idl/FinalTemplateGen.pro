...
;N = 2000L
N = long(((FILE_INFO(maincap)).size-14.0*4.0)/16000.0) - 3
...
for i=0L,N-1L do begin
    ...
    ;if( max(P1[980:1020]*180.0/!Pi) GT 25.0 AND max(P2[980:1020]*180.0/!Pi) GT 25.0) then begin
    ...
    if( count1 LE 8 AND max(P1*180.0/!Pi) GT pm[0] AND max(P1*180.0/!Pi) LT (pm[0]+pm[1])) then begin
      plot,dindgen(801)*1.25 - 125.0,P1[900:1700]*180.0/!Pi,title='Resonator 1',xtitle='Microseconds';,/ystyle,yr=[-20,120],/xstyle,xr=[-100,600]
      plot,dindgen(801)*1.25 - 125.0,A1[900:1700]*180.0/!Pi,title='Resonator 1',xtitle='Microseconds',line=1
      count1++
    ...
    ; export some strip events
    if( count3 LE 20 and max(P1[980:1020]*180.0/!Pi) GT 20.0 AND max(P2[980:1020]*180.0/!Pi) GT 20.0 ) then begin
      openw,3,outpath+'bananapulse.txt',/APPEND
      for k=0,799 do printf,3,P1[900+k]*180.0/!Pi,P2[900+k]*180.0/!Pi
      close,3
      count3++
    endif
    ...
    ; plot noisy bifurcated pulses
    if( i GT 100000L AND i LT 100050L ) then begin
      plot,dindgen(801)*1.25 - 125.0,P2[900:1700]*180.0/!Pi,title='Resonator 2',xtitle='Microseconds',color=100
    endif
