...
if( i EQ 2 ) then begin 
  ...
  ; just for paper - don't use normally!
  ;histzr = 100
  ;Pmax = 55
...
for j=0,n_elements(chisq)-1 do begin
  ;if( i EQ 2 AND chisq[j] GE 1.2 ) then ch1[j] = 0.0
...
plot, bins,hist,psym=10,/ystyle,yr=[0,max(hist)*1.1],xr=[0,Pmax*1.6],/xstyle,xtitle='Pulse Height (degrees)',title=strcompress('Channel' + string(((i mod 2)+1)))
...
print,bestnorm
if( bestnorm EQ 1 ) then bestnorm=1e9;
...
for j=0,19 do begin
  ...
  if( parinfo[0].value GE 150.0) then parinfo[0].value=145.0
