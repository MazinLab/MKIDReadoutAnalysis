...
path = '/Users/benmazin/Data/Projects/MasterPulseAnalysis/'
cd,path
...
if (template EQ '0') then begin
  ...
  ; cheat
  ;pm = [10.0,3.0,13.0,3.0,pm[4]*2.0,pm[5]*2.0]
  ;pm = [120.0,20.0,80.0,20.0,pm[4]*2.0,pm[5]*2.0]
  ;pm = [120.0,15.0,120.0,15.0,0.25,0.25]
  ;pm = [25.0,5.0,28.0,5.0,pm[4]*1.5,pm[5]*1.5]
  ;pm[5]=0.1
...
; pulse fitting
;print,'Pulse Fitting'
;PulseFit,datapath,outpath,pulsename,StartT,atten,res,pm

; generate histograms
print,'Fitting Histograms'      
phr = dblarr(4)
;HistoFit3peak,datapath,outpath,pulsename,t,atten,res,pm,phr
HistoFit,datapath,outpath,pulsename,t,atten,res,pm,phr
EnergyFit,datapath,outpath,pulsename,t,atten,res,pm,phr
...
fitname = strcompress(outpath + string(fix(StartT)) +'-'+ string(fix(res)) +'-'+ string(fix(atten)) +'.ps',/remove_all)
merge = strcompress(fitname + ' ' + outpath+'template.ps ' + outpath + 'template-secondpass.ps ' + outpath + 'strip.ps ' + outpath + 'energyfit.ps ' + outpath + 'ResEstimates.pdf' )
;merge = strcompress(fitname + ' ' + outpath+'template.ps ' + outpath + 'template-secondpass.ps ' + outpath + 'strip.ps ' + outpath + 'histfit.ps '+ outpath + 'energyfit.ps ' + outpath + 'ResEstimates.pdf' ) 
;merge = strcompress(fitname + ' ' + outpath+'template.ps ' + outpath + 'template-secondpass.ps ' + outpath + 'strip.ps ' + outpath + 'histfit.ps ' + outpath + 'ResEstimates.pdf' )
